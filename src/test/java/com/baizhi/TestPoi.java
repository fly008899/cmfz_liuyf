...
public class TestPoi {
    ...
    @Test
    public void testEasypoiExport() {
        //查询数据库返回集合
        ArrayList<Student> stus = new ArrayList<>();
        stus.add(new Student("1","小可爱",16,new Date()));
        stus.add(new Student("2","小蛋黄",16,new Date()));
        stus.add(new Student("3","小狗蛋",12,new Date()));
        stus.add(new Student("4","小嘿嘿",10,new Date()));
        stus.add(new Student("5","小小小",23,new Date()));

        //easypoi导出  参数：Excel表格的title,工作薄的名字 要导出的对象，要导出对象的集合
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生","学生"),Student.class, stus);

        //导出
        try {
            workbook.write(new FileOutputStream(new File("D://TestEasyPoi.xls")));
            workbook.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    @Test
    public void testEasypoiExports() {
        //查询数据库返回集合
        ArrayList<Student> stus = new ArrayList<>();
        stus.add(new Student("1","小可爱",16,new Date()));
        stus.add(new Student("2","小蛋黄",16,new Date()));
        ...
        teachers.add(new Teacher("2","cpx",stus));
        teachers.add(new Teacher("3","hxz",stus));
        ...
        //导出
        try {
            workbook.write(new FileOutputStream(new File("D://TestEasyPois1.xls")));
            workbook.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
